...
namespace PucpConnectPresentacion.templates
{
    public partial class Events : System.Web.UI.Page
    {
        ...
        private List<interes> _todosIntereses;
        ...
        private void CargarIntereses()
        {
            try
            {
                _todosIntereses = _interesWS.listarInteres()?.ToList() ?? new List<interes>();
        ...
        public List<interes> ObtenerInteresesEvento(object eventoItem)
        {
            if (eventoItem is evento ev && ev.idEvento > 0)
            {
                try
                {
                    return _eventoWS.listarInteresesPorEvento(ev.idEvento)?.ToList() ?? new List<interes>();
                }
                catch
                {
                    return new List<interes>();
                }
            }
            return new List<interes>();
        }
